...
const Order = () => {
  ...
  const [ordersUser, setOrdersUser] = useState<TOrder[]>([]);
  const { profile } = useProfile<TMyProfile>();

  useEffect(() => {
    if (orders.length > 0 && profile && profile.data) {
      const ordersInfo = orders.filter((order) => order.cartsOrder.map((cart) => cart.userId === profile.data.id));
      console.log(ordersInfo);

      setOrdersUser(ordersInfo);
    }
  }, [orders, profile]);
    ...
    <>
      ...
      <div className="p-3 shadow-shadow2 mt-8">
        ...
        <div className="list-order flex flex-wrap gap-3 mt-4 w-full">
          {isDefined(ordersUser) ? (
            ordersUser.map((order) =>
              order.cartsOrder.map((cart) => (
